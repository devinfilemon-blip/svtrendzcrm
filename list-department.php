...
function fngetlistdepartment() {
    $.ajax({
        ...
        data: JSON.stringify({ action: 'fngetlistdepartment' }),
        success: function(response) {
            ...
            if (response.status === 'success') {
                ...
                departments.forEach(function(dept, index) {
                    rows += `<tr>
                        <td>${index + 1}</td>
                        <td>${dept.sDepartment}</td>
                        <td>
                            <form action="add-department.php" method="post">
                                <input type="hidden" name="id" value="${dept.iDepid}">
                                <button type="submit" name="btnedit" value="Edit" class="btn btn-success">Edit</button>
                            </form>
                        </td>
                        <td>
                            <button class="btn btn-danger delete-btn" onclick="deletedepartment(${dept.iDepid})">Delete</button>
                        </td>
                    </tr>`;
                });
                ...
                $('#datatable tbody').html('<tr><td colspan="4">No departments found</td></tr>');
            }
        },
        error: function(xhr, status, error) {
