...
<head>
    <title>List  Priority level</title>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            <h4 class="mb-sm-0 font-size-18">List Priority level</h4>

                            <div class="page-title-right">
                                <ol class="breadcrumb m-0">
                                    <li class="breadcrumb-item"><a href="javascript: void(0);"> Priority level</a></li>
                                    <li class="breadcrumb-item active">List  Priority level</li>
                                </ol>
                            </div>
...
function fngetlistpriority() {
    $.ajax({
        url: 'api.php', 
        method: 'POST',
        contentType: 'application/json',
        data: JSON.stringify({
            action: 'fngetlistpriority'
        }),
        success: function(response) {
            ...
            if (response.status === 'success') {
                ...
                statuses.forEach(function(status, index) {
                    statusRows += `<tr>
                                   <td>${index + 1}</td> 
                                   <td>${status.sPrioritylevel}</td>
                                   <td>
                                       <form action="add-priority-level.php" method="post">
                                           <input type="hidden" name="id" value="${status.id}">
                                           <button type="submit" name="btnedit" value="Edit" class="btn btn-success">Edit</button>
                                       </form>
                                   </td>
                                   <td>
                                       <button class="btn btn-danger delete-btn" data-id="${status.id}" onclick="deletepriority(${status.id})">Delete</button>
                                   </td>
                               </tr>`;
                });
