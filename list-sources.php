                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            <h4 class="mb-sm-0 font-size-18">List Sources</h4>

                            <div class="page-title-right">
                                <ol class="breadcrumb m-0">
                                    <li class="breadcrumb-item"><a href="javascript: void(0);">Sources</a></li>
                                    <li class="breadcrumb-item active">List Sources</li>
                                </ol>
                            </div>
...
<script>
function fngetlistsources() {
    $.ajax({
        url: 'api.php',
        method: 'POST',
        contentType: 'application/json',
        data: JSON.stringify({ action: 'fngetlistsources' }),
        success: function(response) {
            ...
            if (response.status === 'success') {
                ...
                sources.forEach(function(source, index) {
                    sourceRows += `<tr>
                        <td>${index + 1}</td>
                        <td>${source.sSources}</td>
                        <td>
                            <form action="add-lead-sources.php" method="post">
                                <input type="hidden" name="id" value="${source.iSourceid}">
                                <button type="submit" name="btnedit" value="Edit" class="btn btn-success">Edit</button>
                            </form>
                        </td>
                        <td>
                            <button class="btn btn-danger delete-btn" onclick="deletesources(${source.iSourceid})">Delete</button>
                        </td>
                    </tr>`;
                });
                ...
                $('#datatable tbody').html(sourceRows);

                // Re-initialize DataTable
                $('#datatable').DataTable();
            } else {
                $('#datatable tbody').html('<tr><td colspan="4">No sources found</td></tr>');
            }
        },
        error: function(xhr, status, error) {
            console.error('AJAX Error:', error);
            alert('An error occurred while fetching sources: ' + error);
        }
    });
}
