...
?>
<head>
    <title>List Offer Letters</title>
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            <h4 class="mb-sm-0 font-size-18">Offer Letters</h4>
                            <div class="page-title-right">
                                <ol class="breadcrumb m-0">
                                    <li class="breadcrumb-item"><a href="javascript: void(0);">HR Letters</a></li>
                                    <li class="breadcrumb-item active">List Offer Letters</li>
                                </ol>
                            </div>
                        </div>
                    </div>
                ...
                <div class="row mb-3">
                    <div class="col-12 text-end">
                        <a href="generate-offer-letter.php" class="btn btn-primary btn-sm">+ Add Offer Letter</a>
                    </div>
                </div>
...
function loadOfferLetters() {
    $.ajax({
        ...
        data: JSON.stringify({ action: 'listofferletters' }),
        success: function(response) {
            var rows = '';
            if (response.status === 'success') {
                var list = response.data || [];
                if (!list.length) {
                    rows = '<tr><td colspan="10" class="text-center">No offer letters found</td></tr>';
                } else {
                    list.forEach(function(r, index) {
                        ...
                        rows += '<td>' + escapeHtml(r.sSalary || '-') + '</td>';
                        rows += '<td><a class="btn btn-info btn-sm" target="_blank" href="print-offer-letter.php?id=' + r.iLetterid + '">Print</a></td>';
                        rows += '<td><form action="generate-offer-letter.php" method="post">' +
...
function deleteLetter(id) {
    if (!id || !confirm('Delete this offer letter?')) return;
...
$(document).ready(loadOfferLetters);
</script>
</body>
</html>
